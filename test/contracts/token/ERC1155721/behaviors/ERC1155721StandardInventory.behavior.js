        ...
        describe("ERC721 functions on non-NFT ids", function() {
            ...
            describe("approve", function() {
                ...
                context("applied on a fungible token id", function() {
                    it("reverts", async function() {
                        await expectRevert(
                            this.token.approve(approved, fCollection1.id),
                ...
                context("when successful", function() {
                    const transferFrom = function(from) {
                        ...
                        it("should increase the non-fungible token balance of the new owner", async function() {
                            (await this.token.balanceOf(this.toWhom, nft1)).should.be.bignumber.equal(
                                this.nftBalanceToWhom.addn(1)
                            );
                        });
    
                        it("should decrease the non-fungible token balance of the previous owner", async function() {
                            (await this.token.balanceOf(owner, nft1)).should.be.bignumber.equal(
                                this.nftBalanceOwner.subn(1)
                            );
                        });
                ...
                context("when successful", function() {
                    ...
                    const batchTransferFrom = (from, approvedSender = false) => {
                        beforeEach(async function() {
                            ...
                            for (const item of batch) {
                                if (item.isFungible) {
                                    if (!approvedSender) {
                                        ...
                                        this.fCollectionBalanceToWhom.push(
                                            await this.token.balanceOf(this.toWhom, item.id)
                                        );
                                    ...
                                    this.nfCollectionBalanceOwner.push(
                                        await this.token.balanceOf(owner, item.nfCollection)
                                    );
                                    this.nfCollectionBalanceToWhom.push(
                                        await this.token.balanceOf(this.toWhom, item.nfCollection)
                                    );
                            ...
                            this.receipt = await this.token.methods[method](
                                owner,
                                this.toWhom,
                                this.tokens,
                                this.values,
