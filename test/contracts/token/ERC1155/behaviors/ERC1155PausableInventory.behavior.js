    ...
    describe('Pausable', function () {
      ...
      context('when paused', function () {
        ...
        it('blocks setApprovalForAll', async function () {
          await expectRevert(this.token.setApprovalForAll(operator, true, {from: owner}), Paused_RevertMessage);
        });
        ...
        it('blocks burnFrom', async function () {
          await expectRevert(this.token.burnFrom(owner, fCollection2.id, '1', {from: owner}), Paused_RevertMessage);
          await expectRevert(this.token.burnFrom(owner, nft1, '1', {from: owner}), Paused_RevertMessage);
          await expectRevert(this.token.burnFrom(owner, nft3, '1', {from: owner}), Paused_RevertMessage);
          await expectRevert(this.token.burnFrom(owner, fCollection1.id, '1', {from: owner}), Paused_RevertMessage);
        });
      });
    });
  });
